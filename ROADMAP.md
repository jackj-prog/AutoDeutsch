...
## ▶ Current status (2026-06-22) — read `HANDOFF.md` first, then `docs/NEXT-HORIZON.md`
The big programs are **all shipped** (live build `2026.06.20.50`); this is a map — full detail in `CHANGELOG.md`:
- ✅ **Journey build-out J1–J6** (v.23–28) — `docs/JOURNEY-ROADMAP.md`.
- ✅ **"Next Horizon" H1–H4** (v.29–36) — stabilize/modularization · voice-first roleplay · retention ·
  content depth (all 58 missions multi-scene). `docs/NEXT-HORIZON.md`.
- ✅ **AI Tutor** — fully built: context-aware, the **"Do it for real" roleplay** (Tutor plays the other
  party, spoken in/out), and it can launch weak-word drills. *(The "Strategic note" below is obsolete.)*
- ✅ **Commercial-grade C1–C7** (v.38–44) — About&Privacy · German proofread · first-run pitch · offline AI ·
  share cards · a11y · install prompt.
- ✅ **Journey visual redesign VJ1–VJ6** (v.45–50) — Scenarios is now a premium centered-timeline **map**
  (HANDOFF → "Journey map architecture").

**What's left:** nothing blocking. Pick from `HANDOFF.md` → "Open hand-offs".

## Strategic note (HISTORICAL — superseded; the AI Tutor is now built)
...
| **P2** | **Can-do CEFR + capability Progress.** Progress leads with **"What you can do now"** (earned can-dos from completed missions) + per-level telc exam-readiness chips + can-do counts; the word-count / memory-strength / drill panels are demoted under a collapsible "Under the hood." Level certificates deferred to P4. | CEFR was a word-bucket label, not a can-do journey; Progress measured completion, not ability. | Med-High | Med | ✅ **DONE v2026.06.20.07** (P2a; certificates → P4) |
| **P3** | **Daily loop = forward mission step.** Home now shows the mission's concrete next step + relocation status (P3a). Full goal-reframe (cards/day → mission-step/minutes; reviews fully secondary) deferred as P3b (touches the retention hook). | The daily experience was debt-servicing + grinding cards, not advancing. | Med | Med | 🟡 **P3a DONE v2026.06.20.09** (forward step + status on Home; P3b goal-reframe pending) |
| **P4** | **Reward & milestone re-wire.** Capability rewards + role/identity progression. DACH-rite milestones (Krankenversicherung, GEZ, Mietkaution, Steuer-ID) need new scenario content → other agent's lane. | Achievements were disconnected from real-world capability; rewards fired on counts, not competence. | Med | Low-Med | 🟡 **P4a DONE v2026.06.20.08** (rewards+identity; DACH-rite milestones pending content) |
| **P5** | **Narrative onboarding + placement + personalization.** First-run is now intake (country/reason/timeline/goal) → a 2-min placement test → a result screen with the estimated level + a personalized roadmap line + the first mission. Retake via Settings → Placement test. | Onboarding was a 3-field config that set the "tool" frame; it never invoked the product's superpower. | Med-High | Med | ✅ **DONE v2026.06.20.11** (built from the content lane's PLACEMENT data + hand-off spec) |
| **P6** | **Library → Curriculum; taxonomy → "Dictionary."** Re-present the main content surface as the sequenced mission map; keep the 36-category taxonomy + 5,782-row Browse as an explicitly-labelled reference, not the front door. | Library/Browse expose the raw database as the primary content surface; discovery is by word-category, not life-situation. | Med | Low-Med | ✅ done (v.18): Library now opens on the journey — a continue-hero (`currentMission`) + the 7-arc map with progress above the fold; the 36-category taxonomy + Browse are demoted under an explicit **"Dictionary — a reference, not a path"** divider. Reuses the Scenarios/Mission screens (no duplication). Spec: `docs/P6-curriculum-spec.md`. |
| **P7** | **Tutor as narrative guide (later).** The coach who walks you through the journey and role-plays each scenario — not a generic chatbot bolted on. | Owns narrative, purpose, momentum. Deferred per direction until the core journey exists and demand is validated. | High | Med | 🟡 increment 1 shipped (v.17): the existing BYO-key Tutor is now **context-aware** — its system prompt + starter chips are built from the learner's live level, relocation goal, status, current scenario and weak words (`docs/P7-tutor-context-spec.md`). Full productisation (hosted proxy, drill-launching) still deferred. |
...
**A1 on-ramps + practice datasets (content lane, 2026-06-20):** every arc now has an **A1 way in** — 5 new A1 "einfach" entry scenes (cash/ATM, feeling unwell, booking an appointment, the flat, making plans) wired into the earliest mission of the money/health/paperwork/roof/belonging arcs, plus a dedicated A1 `cash` mission (money previously started at B1). Three new ready-to-wire datasets shipped in `data.js`: **`CONFUSIONS`** (20 confusion pairs → confusion-pair drill, Learning #9), **`EXAM`** (12 telc/Goethe-style Leseverstehen/Sprachbausteine sets, A1–B2 → exam mode, Learning #10), and **`PLACEMENT`** (P5, 24 graded items + intake). Validator + screenshot-harness paths extended to cover all three; specs in `docs/practice-data-spec.md`, `docs/P5-placement-spec.md`, `docs/P5-placement-buildspec.md`. Plus 4 deeper relocation scenes (Überweisung, Nebenkostenabrechnung, Nachbarn, Bewerbungs-Nachfrage) → missions `nebenkosten`/`neighbours`/`apply`. Dialogues → 112, missions → 48.
...
| 9 | Confusion-pair drilling | 🟡 | Data ready: `CONFUSIONS` (20 pairs × 4 gapped items + `why`) in data.js; drill UI pending (app.jsx). Spec: docs/practice-data-spec.md. |
| 10 | Exam-format practice | 🟡 | Data ready: `EXAM` (12 telc/Goethe-style sets, A1–B2, Leseverstehen + Sprachbausteine) in data.js; exam UI pending (app.jsx). Spec: docs/practice-data-spec.md. |
