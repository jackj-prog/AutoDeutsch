...
**A1 on-ramps + practice datasets (content lane, 2026-06-20):** every arc now has an **A1 way in** — 5 new A1 "einfach" entry scenes (cash/ATM, feeling unwell, booking an appointment, the flat, making plans) wired into the earliest mission of the money/health/paperwork/roof/belonging arcs, plus a dedicated A1 `cash` mission (money previously started at B1). Three new ready-to-wire datasets shipped in `data.js`: **`CONFUSIONS`** (14 confusion pairs → confusion-pair drill, Learning #9), **`EXAM`** (6 telc/Goethe-style Leseverstehen/Sprachbausteine sets → exam mode, Learning #10), and **`PLACEMENT`** (P5, above). Validator extended to cover all three; specs in `docs/practice-data-spec.md` + `docs/P5-placement-spec.md`.
...
| 9 | Confusion-pair drilling | 🟡 | Data ready: `CONFUSIONS` (14 pairs × 4 gapped items + `why`) in data.js; drill UI pending (app.jsx). Spec: docs/practice-data-spec.md. |
| 10 | Exam-format practice | 🟡 | Data ready: `EXAM` (6 telc/Goethe-style sets, Leseverstehen + Sprachbausteine) in data.js; exam UI pending (app.jsx). Spec: docs/practice-data-spec.md. |
